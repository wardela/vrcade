...
import DeleteBalanceModal from "./deletebalanceomodal";
import ReceiptVoucherModal from "./ReceiptVoucherModal";
import CreateReceiptModal from "./CreateReceiptModal";
import DeleteReceiptVoucherModal from "./DeleteReceiptVoucherModal";
import DueBalancePrint from "./DueBalancePrint";
...
export default function ReceiptsScreen() {
  ...
  const [deleteBalanceId, setDeleteBalanceId] = useState(null);
    ...
    const [activeRvClient, setActiveRvClient] = useState("");
...
const formatDate = (dateStr) => {
  ...
  return t(`Receipts.status.${state}`, state);
};

  const toggleExpand = (dueId) => {
    if (expandedDue === dueId) {
      setExpandedDue(null);
      return;
    }

    setExpandedDue(dueId);
...
const handlePrint = useReactToPrint({
  ...
  documentTitle: printRV?.receipt_number ? `RV-${printRV.receipt_number}` : "Receipt Voucher",
  pageStyle: `
...
}, [pendingPrint, printRV, company, handlePrint]);

const printReceiptVoucher = async (rvId) => {
  try {
    // fetch full RV details (includes cheques if type=cheque)
    const res = await api.get(`/api/invoices/receipt-vouchers/${rvId}`);
    setPrintRV(res.data);
    setPendingPrint(true);
  } catch (err) {
    console.error("Failed to fetch receipt voucher for print", err);
  }
};
...
return (
  <div className="flex flex-col h-screen bg-gradient-to-br from-gray-50 to-gray-100/50">
    ...
    <div className="flex-shrink-0 bg-white/80 backdrop-blur-sm border-b border-gray-200/60 shadow-sm">
      {/* Header */}
      <div className="px-6 pt-5 pb-4 border-b border-gray-200/60">
        ...
        <div className="flex flex-wrap gap-6 items-end">
          <div className="flex-1 min-w-[200px]">
            <label className="block text-xs font-medium text-gray-600 mb-2">
              {t("Receipts.filters.client")}
            </label>
            <button
              onClick={() => setClientModalOpen(true)}
              className="w-full px-4 py-2.5 border border-gray-300 rounded-lg bg-white hover:bg-gray-50 hover:border-gray-400 transition-all duration-200 text-start text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
            >
              {selectedClient ? selectedClient.name : t("Receipts.filters.select_client")}
            </button>
          </div>
          ...
          {!loading &&
            data.map((d) => (
              ...
              >
                {/* Due Row */}
                <div
                  className="grid grid-cols-7 gap-4 p-5 cursor-pointer hover:bg-gray-50/80 transition-colors duration-150"
...
<div className="flex flex-col">
  <span className="text-sm text-gray-700">{d.client}</span>
</div>
