...
export default function ReceiptVoucherModal({
  ...
  // =========================
  // Load RV in edit mode
  // =========================
  useEffect(() => {
    if (mode === "edit" && receiptVoucherId) {
      fetchVoucherDetails();
    } else if (mode === "create") {
      resetForm();
    }
  }, [mode, receiptVoucherId]);
  ...
  const fetchVoucherDetails = async () => {
    ...
    try {
      ...
      console.error("Error loading receipt voucher:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    setLoading(true);
    try {
      let rvId = receiptVoucherId;

      if (mode === "create") {
        const rvRes = await api.post("/api/invoices/receipt-vouchers", {
  due_balance_id: dueBalanceId,
  date: form.date,
  type: form.type,
  ...
  notes: form.notes
        ...
        await api.put(`/api/invoices/receipt-vouchers/${receiptVoucherId}`, {
  date: form.date,
  type: form.type,
  amount: Number(form.amount),
  reason: form.reason,
  notes: form.notes
      ...
      if (form.type === "cheque") {
        for (const ch of cheques) {
          await api.post("/api/invoices/receipt-cheques", {
            receipt_voucher_id: rvId,
            cheque_number: ch.cheque_number,
            cheque_amount: ch.cheque_amount,
            due_date: ch.due_date,
            beneficiary_bank: ch.beneficiary_bank
          });
        }
      }
...
// Can user write in this modal mode?
const canWriteThisMode = mode === "create" ? canAdd : canEdit;

// Read-only if user cannot write in this mode
const isReadOnly = !canWriteThisMode;

// Show submit only if user can write in this mode
        ...
        >
          ...
          <div className="px-6 pt-6 pb-5 border-b border-gray-200/60">
            <h2 className="text-xl font-semibold text-gray-900 tracking-tight">
              {mode === "create"
                ? t("ReceiptVoucherModal.title.create")
                : t("ReceiptVoucherModal.title.edit")}
            </h2>
            <p className="text-sm text-gray-500 mt-1">
              {mode === "create" 
                ? `${t("ReceiptVoucherModal.subtitle.create")} ${clientName}`
                : `${t("ReceiptVoucherModal.subtitle.edit")} ${clientName}`
              }
            </p>
          </div>
          ...
          <div className="px-6 py-5 space-y-5 max-h-[calc(100vh-280px)] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-2">
                  {t("ReceiptVoucherModal.fields.receipt_number")}
                </label>
                <input
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-2">
                  {t("ReceiptVoucherModal.fields.date")} <span className="text-red-500">*</span>
                </label>
                <input
                  type="date"
                  value={form.date}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-2">
                  {t("ReceiptVoucherModal.fields.payment_type")} <span className="text-red-500">*</span>
                </label>
                <select
                  value={form.type}
                ...
                >
                  <option value="cash">{t("ReceiptVoucherModal.payment_types.cash")}</option>
                  <option value="bank_transfer">{t("ReceiptVoucherModal.payment_types.bank_transfer")}</option>
                  <option value="cheque">{t("ReceiptVoucherModal.payment_types.cheque")}</option>
                </select>
              </div>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-2">
                  {t("ReceiptVoucherModal.fields.amount")} <span className="text-red-500">*</span>
                </label>
            ...
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-2">
                 {t("ReceiptVoucherModal.fields.reason")}
              </label>
              <input
                type="text"
                placeholder={t("ReceiptVoucherModal.placeholders.reason")}
                value={form.reason}
            ...
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-2">
                {t("ReceiptVoucherModal.fields.notes")}
              </label>
              <textarea
                rows={3}
                placeholder={t("ReceiptVoucherModal.placeholders.notes")}
                value={form.notes}
            ...
            {form.type === "cheque" && (
              <div className="space-y-4 pt-2">
                <div className="flex items-center justify-between">
                  <div>
                    <label className="block text-xs font-medium text-gray-700">
                      {t("ReceiptVoucherModal.cheques.section_title")} <span className="text-red-500">*</span>
                    </label>
                    <p className="text-xs text-gray-500 mt-1">
                      {t("ReceiptVoucherModal.cheques.section_hint")}
                    </p>
                  </div>
                  <button
                    onClick={addCheque}
                  ...
                  >
                    +  {t("ReceiptVoucherModal.cheques.add")}
                  </button>
                </div>
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-semibold text-gray-700">
                          {t("ReceiptVoucherModal.cheques.index")} - {i + 1}
                        </span>
                        {cheques.length > 1 && (
                          <button
                            onClick={() => {
                          ...
                          >
                            {t("ReceiptVoucherModal.cheques.remove")}
                          </button>
                        )}
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                          <label className="block text-xs font-medium text-gray-600 mb-1.5">
                            {t("ReceiptVoucherModal.cheques.fields.number")}
                          </label>
                          <input
                            type="text"
                            placeholder="CHQ-123456"
                            value={ch.cheque_number}
                            onChange={(e) => updateCheque(i, "cheque_number", e.target.value)}
                            disabled={loading || isReadOnly}
                        ...
                        <div>
                          <label className="block text-xs font-medium text-gray-600 mb-1.5">
                            {t("ReceiptVoucherModal.cheques.fields.amount")}
                          </label>
                          <div className="relative">
                            ...
                            <input
                              ...
                              onChange={(e) => updateCheque(i, "cheque_amount", e.target.value)}
                              disabled={loading || isReadOnly}
                        ...
                        <div>
                          <label className="block text-xs font-medium text-gray-600 mb-1.5">
                            {t("ReceiptVoucherModal.cheques.fields.due_date")}
                          </label>
                          <input
                            type="date"
                            value={ch.due_date}
                            onChange={(e) => updateCheque(i, "due_date", e.target.value)}
                            disabled={loading || isReadOnly}
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 disabled:bg-gray-50 disabled:cursor-not-allowed"
                          />
                        </div>
                        ...
                        <div>
                          <label className="block text-xs font-medium text-gray-600 mb-1.5">
                            {t("ReceiptVoucherModal.cheques.fields.bank")}
                          </label>
                          <input
                            type="text"
                            placeholder={t("ReceiptVoucherModal.cheques.placeholders.bank")}
                            value={ch.beneficiary_bank}
                            onChange={(e) => updateCheque(i, "beneficiary_bank", e.target.value)}
                            disabled={loading || isReadOnly}
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 disabled:bg-gray-50 disabled:cursor-not-allowed"
                          />
                        </div>
                ...
                {(() => {
                  ...
                  const isValid = Math.abs(chequeTotal - receiptAmount) < 0.01;
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className={`text-sm font-medium ${
                            ...
                            {t("ReceiptVoucherModal.cheques.validation.total")} {chequeTotal.toFixed(2)}
                          </p>
                          <p className={`text-xs ${
                            isValid ? 'text-green-600' : 'text-amber-600'
                          }`}>
                            {isValid 
                              ? t("ReceiptVoucherModal.cheques.validation.match")
                              : `${t("ReceiptVoucherModal.cheques.validation.mismatch")} (${receiptAmount.toFixed(2)})`
                            }
                          </p>
                        </div>
                      </div>
          ...
          <div className="px-6 py-4 bg-gray-50/80 border-t border-gray-200/60 flex justify-between items-center gap-3">
            <p className="text-xs text-gray-500">
              <span className="text-red-500">*</span> {t("ReceiptVoucherModal.footer.required")}
            </p>
            <div className="flex gap-3">
              ...
              >
                {t("ReceiptVoucherModal.buttons.cancel")}
              </button>
              {showSubmit && (
              <button
                disabled={(() => {
                  // Basic validation
                  if (loading || !form.amount || !form.date) return true;
              ...
              >
                ...
                {loading 
                  ? t("ReceiptVoucherModal.buttons.saving")
                  : mode === "create" 
                    ? t("ReceiptVoucherModal.buttons.create")
                    : t("ReceiptVoucherModal.buttons.save")}
              </button>
              )}
            </div>
          </div>
