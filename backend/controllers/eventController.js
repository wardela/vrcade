...
const updateEventAmount = async (req, res) => {
  try {
    const event = await eventService.updateEventAmount(req.db, req.params.id, req.body);
    res.status(200).json(event);
  } catch (error) {
    console.error("Error updating event amount:", error);
    if (error?.statusCode) {
      return res.status(error.statusCode).json({ message: error.message });
    }
    res.status(500).json({ message: getApiMessage(error, "Failed to update event amount") });
  }
};

const updateEventStatus = async (req, res) => {
  try {
    const event = await eventService.updateEventStatus(req.db, req.params.id, req.body);
    res.status(200).json(event);
  } catch (error) {
    ...
    res.status(500).json({ message: getApiMessage(error, "Failed to update event status") });
  }
};
