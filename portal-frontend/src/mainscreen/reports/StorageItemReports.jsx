...
useEffect(() => {
  api.get(`/api/portal/reports/storages`)
  ...
  const fetchItemsSales = async (newOffset = 0) => {
    ...
    try {
      ...
      const res = await api.get(
        `/api/portal/reports/items/sales`,
        {
          params: {
            from: dateFrom,
            to: dateTo,
...
const applyReport = () => {
  ...
  try {
    ...
    const res = await api.get(
      `/api/portal/reports/items/sales/details`,
      {
        params: {
...
const fetchStorageInventory = async () => {
  ...
  try {
    ...
    const res = await api.get(
      `/api/portal/reports/storage/inventory`,
...
const fetchStorageTransactions = async (newOffset = 0) => {
  ...
  try {
    ...
    const res = await api.get(
      `/api/portal/reports/storage/transactions`,
      {
        params: {
          from: dateFrom,
          to: dateTo,
